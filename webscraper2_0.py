...
# Extract announcements from the main page
extract_announce()

# Navigate to books page
try:
    # Look for menu items by text content rather than specific ID
    target_link = driver.find_element(By.XPATH, '//a[contains(text(), "Books") or contains(@href, "falun-dafa-books.html")]')
    time.sleep(1)
    driver.execute_script("arguments[0].click();", target_link)
    time.sleep(2)  # Increased wait time
    print("Navigated to Books and Recent Writings page")
except Exception as e:
    print("Could not find Books link:", e)
    # Print current URL to debug
    print("Current URL:", driver.current_url)

# Navigate to Recent Writings section if needed
try:
    # Try to find "Recent Writings" link with a more general approach
    target_link = driver.find_element(By.XPATH, '//a[contains(text(), "Recent Writings")]')
    driver.execute_script('arguments[0].click();', target_link)
    time.sleep(2)
    print("Navigated to Recent Writings section")
except Exception as e:
    print("Recent Writings link not found:", e)
    # Print current URL to debug
    print("Current URL:", driver.current_url)

def extract_recent():
    print("Extracting recent writings...")
    print("Current URL:", driver.current_url)
    ...
    try:
        ...
        # More general approach to find book titles
        book_titles = driver.find_elements(By.XPATH, '//div[contains(@class, "row-book")]//h3')

        if book_titles:
            print(f"Found {len(book_titles)} book titles")
            ...
            books = soup.select('.row-book h3')

            if books:
                print(f"Found {len(books)} book titles using BeautifulSoup")
                for book in books:
                    print(f"Book Title: {book.text.strip()}")
            else:
                print("No books found with BeautifulSoup either")

    except Exception as e:
        print(f"Error in extract_recent: {e}")

# Call the function to extract recent writings
extract_recent()
